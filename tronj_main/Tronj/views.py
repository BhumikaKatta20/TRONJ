...
cod_base=cod[cod['Department'] == 'web']
randomlist=[]
first=second=third=fourth=fifth=sec_first=sec_second=sec_third=fifth=sixth=seventh=eighth=ninth=tenth=0
...
def posted_job(request,id):
	...
	if sk_lng=="PHP":
		sk = 'PHP Programming'
	backgrd="PL4.jpg"
	exp=company.years_of_experience
	if exp=="Fresher":
		skexp=exp;
	else:
		skexp=str(exp)+"years"
	return render(request,'Tronj/company/job_posted.html',{'backgrd':backgrd,'c':company,
		'sk':sk,'sk_lng':sk_lng,'skexp':skexp})
def index(request):
	import sqlite3
	DBgame = sqlite3.connect('AmBhu_db.sqlite3')
	game = DBgame.cursor()
	daa=game.execute("SELECT * from Tronj_company_details;")
	DBgame.commit()
	print(daa)
	...
	plt.savefig("Tronj/static/app/images/job_avii.png")
	plt.savefig('outpu.png')
	headlingcontent = 'color: black'
	DBgame.close()
	print("In index functionn")
	return render(request,'index.html',{'backgrd':backgrd,
		'parcontent':parcontent,'headlingcontent':headlingcontent,'graph':graph})
def del_user(request):
	backgrd="PL4.jpg"
	if request.user.is_staff:
		name = request.user
		company_details.objects.filter(Company_name=name).delete()
		Avilable_work.objects.filter(company=name).delete()
	else:
		name = request.user
		Avilable_work.objects.filter(Employee=name).delete()
	print(name)
	...
	#fig, ax = plt.subplots()
	#dat[4].value_counts().plot(ax=ax, kind='pie')
	#plt.savefig("Tronj/static/app/images/job_avi.png")
	#lt.savefig('outpute.png')
	#headlingcontent = 'color: black'
	#DBgame.close()
...
def	Del_job(request):
	...
	ida=request.GET['ida']
	company_details.objects.filter(id=ida).delete()
	Avilable_work.objects.filter(job_id=ida).delete()
	return render(request,'Tronj/company/Capplication.html',{'backgrd':backgrd,'Cdata':Cdata})
...
def sub(request):
	...
	ed = '''<table border="1" cellspacing="4" cellpadding="2" style="margin-left:2%;"><tr style="font-size:20px;margin:10px 10px 10px 10px"><th>Educatoin details</th><th>Institute</th><th>Board</th><th>Passed out Year</th><th>Percentage</th>'''
	SSLC = '''<tr style="font-size:13px;"><td>SSLC</td><td>'''+Fdataa.SSLC_name+'''</td><td>'''+Fdataa.SSLC_board+'''</td><td>'''+str(Fdataa.SSLC_passed_year)+'''</td><td>'''+str(Fdataa.SSLC_Percentage)+'''</td></tr>'''
	PUC = '''<tr style="font-size:13px;"><td>PUC ('''+Fdataa.PUC_IN+''')</td><td>'''+Fdataa.PUC_name+'''</td><td>'''+Fdataa.PUC_board+'''</td><td>'''+str(Fdataa.PUC_passed_year)+'''</td><td>'''+str(Fdataa.PUC_Percentage)+'''</td></tr>'''
	DEGREE = '''<tr style="font-size:13px;">'''+Fdataa.Graduation_IN+'''</td><td>'''+Fdataa.Graduation_name+'''</td><td>'''+Fdataa.Graduation_board+'''</td><td>'''+str(Fdataa.Graduation_passed_year)+'''</td><td>'''+str(Fdataa.Graduation_Percentage)+'''</td></tr>'''
	PostDEGREE = '''<tr style="font-size:13px;"><td>'''+Fdataa.PostGraduation_IN+'''</td><td>'''+Fdataa.PostGraduation_name+'''</td><td>'''+Fdataa.PostGraduation_board+'''</td><td>'''+str(Fdataa.PostGraduation_passed_year)+'''</td><td>'''+str(Fdataa.PostGraduation_Percentage)+'''</td></tr></table>'''
	...
	temp = Avilable_work(company=Company_name,status=status,stipend=stippend,Employee=Employeee,Employee_email=request.user.email,
						   ...
						   Skills_Required=Skills_Required,CEmail=CEmail,job_id=ida
	)
	temp.save()
	...
	return render(request,'Tronj/Employee/Eappli.html',{'backgrd':backgrd,'Cdata':Cdata})

def myAccount(request):
    CO=ST=FN=AD=CT=STA=PCO=PN=DA=0
    if request.user.is_staff:
        Fdata = Company_Profile.objects.get(user=request.user)
        ...
        Fdataa=emp_Education.objects.get(user=request.user)
        backgrd='grey'
        CO=len(Fdata.Carrer_Objective)
        ST=len(Fdata.Strength)
        FN=len(Fdata.Father_Name)
        DA=Fdata.Date_of_birth
    AD=len(Fdata.Address)
    CT=len(Fdata.city)
    STA=len(Fdata.State)
    PCO=Fdata.Postal_code
    PN=Fdata.Phone_number
    cont={'user':request.user,'Fdataa':Fdataa,'Fdata':Fdata,'backgrd':backgrd,'CO':CO,'ST':ST,'FN':FN,
    'AD':AD,'CT':CT,'STA':STA,'PCO':PCO,'PN':PN,'DA':DA}
...
def round1(request):
	...
	if (predate == de_date or de_date < valid_chek_date) and trail<4:
		...
		print(base)
		print("Length is",len(base))
		global randomlist
		randomlist = []
		while len(randomlist) <= 9:
		...
		print("*________*_________*____________*________")
		print(len(base))
		print(randomlist)
		global first, second, third, fourth, fifth,sixth,seventh,eighth,ninth,tenth
		first = randomlist[0]
		second = randomlist[1]
		third = randomlist[2]
		fourth = randomlist[3]
		...
		Q9 = Quiz_data()
		Q10 = Quiz_data()
		qlist=['Ql1','Ql2','Ql3','Ql4','Ql5','Ql6','Ql7','Ql8','Ql9','Ql10']
		cql=[first, second, third, fourth, fifth,sixth,seventh,eighth,ninth,tenth]
		for qval in qlist:
		    for coun in cql:
		        qval = Quiz_data()
		        qval.question = base.loc[coun, 'Question']
		        qval.option1 = base.loc[coun, 'op1']
		        qval.option2 = base.loc[coun, 'op2']
		        qval.option3 = base.loc[coun, 'op3']
		        qval.option4 = base.loc[coun, 'op4']
		        qval.group = str(qval)
		...
		Q4.option4 = base.loc[fourth, 'op4']#fifth,sixth,seventh,eighth,ninth,tenth
		Q4.group = 'group4'
		...
		Q5.group = 'group5'
		Q6.question = base.loc[sixth, 'Question']
		Q6.option1 = base.loc[sixth, 'op1']
		Q6.option2 = base.loc[sixth, 'op2']
		Q6.option3 = base.loc[sixth, 'op3']
		Q6.option4 = base.loc[sixth, 'op4']
		Q6.group = 'group6'
		Q7.question = base.loc[seventh, 'Question']
		Q7.option1 = base.loc[seventh, 'op1']
		Q7.option2 = base.loc[seventh, 'op2']
		Q7.option3 = base.loc[seventh, 'op3']
		Q7.option4 = base.loc[seventh, 'op4']
		Q7.group = 'group7'
		Q8.question = base.loc[eighth, 'Question']
		Q8.option1 = base.loc[eighth, 'op1']
		Q8.option2 = base.loc[eighth, 'op2']
		Q8.option3 = base.loc[eighth, 'op3']
		Q8.option4 = base.loc[eighth, 'op4']
		Q8.group = 'group8'
		Q9.question = base.loc[ninth, 'Question']
		Q9.option1 = base.loc[ninth, 'op1']
		Q9.option2 = base.loc[ninth, 'op2']
		Q9.option3 = base.loc[ninth, 'op3']
		Q9.option4 = base.loc[ninth, 'op4']
		Q9.group = 'group9'
		Q10.question = base.loc[tenth, 'Question']
		Q10.option1 = base.loc[tenth, 'op1']
		Q10.option2 = base.loc[tenth, 'op2']
		Q10.option3 = base.loc[tenth, 'op3']
		Q10.option4 = base.loc[tenth, 'op4']
		Q10.group = 'group5'
		...
		backgrd = 'back10.jpg'
		Question_set = [Q1, Q2, Q3, Q4, Q5,Q6,Q7,Q8,Q9,Q10]
		#Question_set = [Ql1,Ql2,Ql3,Ql4,Ql5,Ql6,Ql7,Ql8,Ql9,Ql10]
...
def round2(request):
    ...
    try:
        ...
        Ans5='Nothing'
    try:
        Ans6=request.GET['group6']
    except:
        Ans6='Nothing'
    try:
        Ans7=request.GET['group7']
    except:
        Ans7='Nothing'
    try:
        Ans8=request.GET['group8']
    except:
        Ans8='Nothing'
    try:
        Ans9=request.GET['group9']
    except:
        Ans9='Nothing'
    try:
        Ans10=request.GET['group10']
    except:
        Ans10='Nothing'
    ...
    if Ans5==base.loc[fifth,'Answer']:
        marks=marks+20
    if Ans6==base.loc[sixth,'Answer']:
        marks=marks+20
    if Ans7==base.loc[seventh,'Answer']:
        marks=marks+20
    if Ans8==base.loc[eighth,'Answer']:
        marks=marks+20
    if Ans9==base.loc[ninth,'Answer']:
        marks=marks+20
    if Ans10==base.loc[tenth,'Answer']:
        marks=marks+20
    print(marks)
    scored=int((marks/200)*100)
	#print('score:',scored)
    tem=skilltest.objects.get(user=request.user)
    ...
    Fdata=Employee.objects.get(user=request.user)
    sk = skilltest.objects.get(user=request.user)
    return render(request,'Tronj/quiz/result.html',{'Fdata':Fdata,'backgrd':backgrd,'user':request.user,
	'scored':scored,'lang':lang,'Fdat':sk})
...
def Empeditpre(request):
	...
	if len(a) > 0 or len(Carrer_Objective)<100:
		flag = True
		Carrer_Objective_error = 'numbers, @ , !, %, $, ^ #, * and less than 100 Characters or too short are not vaild'
	...
	if flag==True:
		...
		return render(request, 'Tronj/edit_Profile.html', {'backgrd': backgrd, 'Fdata': Fdata,
					...
					,'Strength_error':Strength_error,'email_error':email_error})
	else:
...
def	updatee(request):
	...
	email = '1220tronj@gmail.com'
	password = 'Trobhuam#@1.'
	send_to_email = 'Bhumika.katta20@gmail.com'
	Fd = Avilable_work.objects.get(id=ida)
	company_name=str(Fd.company)
	#app_up = .objects.filter(id=ida)
	
	msg = MIMEMultipart()
	msg['From'] = email
	msg['To'] = Fd.CEmail	
	message="Your application number:\t"+ida+"\nApplication Status:\t"+status+"\n"+info
	message="""<html><body><h1 style=" font-family: Copper black;color: #EAA90B;">TRONJ</h1>
        
        <p style=" font-family: Baskerville Old Face;font-size:20px;color:black">We are happy to inform you. There is an Update in your application for <strong>"""+company_name+"""</strong>
        <br>Application Status:<strong>"""+status+"""</strong></p><br><br><br>
        <p style=" font-family: Baskerville Old Face;font-size:20px;color:black"><br><br>"""+info+"""</p>
    </body>
</html>"""
	# subj=Employee+'resume for work based on:'+Skills_Required
	# print(subj)
